...
//		sHeight = res.getDisplayMetrics().heightPixels;
		...
		if (Build.VERSION.SDK_INT >= 19) {
			...
			int paddingTop = mTitleBar.getPaddingTop();
			int paddingBottom = mTitleBar.getPaddingBottom();
			int paddingLeft = mTitleBar.getPaddingLeft();
			int paddingRight = mTitleBar.getPaddingRight();
			mTitleBar.setPadding(paddingLeft, paddingTop + statusBarHeight, paddingRight, paddingBottom);
		}
...
//				mIVSettingsBG.getLayoutParams().height = settingsHeight;
		...
		mLGraphicSurface.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View arg0) {
				Toast.makeText(PerformanceActivity.this, getString(R.string.menu_settings_description), Toast.LENGTH_SHORT).show();
				return true;
			}
		});
...
//				yBottom = (int) (getHeight()*0.88);
		...
		mBChooseProcess.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent i = new Intent(PerformanceActivity.this, ActivityProcesses.class);
		...
		mCloseSettings.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (PerformanceActivity.this.intervalRead != intervalRead) {
				...
				PerformanceActivity.this.intervalRead = intervalRead;
				PerformanceActivity.this.intervalUpdate = intervalUpdate;
				PerformanceActivity.this.intervalWidth = intervalWidth;
		...
		if (savedInstanceState != null && !savedInstanceState.isEmpty()) {
			...
			if (savedInstanceState.getBoolean(C.menuShown))
				mTitleBar.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
					@Override
					public void onGlobalLayout() {
						mTitleBar.getViewTreeObserver().removeGlobalOnLayoutListener(this);
						mPWMenu.showAtLocation(mTitleBar, Gravity.TOP | Gravity.RIGHT, 0, mTitleBar.getHeight());
					}
				});
		}
